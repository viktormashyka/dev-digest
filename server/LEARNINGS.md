...
**2026-08-26 addendum — resolving the actual `feat/multi-agent-review` × `feat/export-to-ci` merge: renumbering a colliding migration file is safe post-hoc IF you also fix the snapshot's `prevId` chain, not just the filename.** Both branches had independently claimed idx `0023` against this same shared DB (`0023_flimsy_nick_fury.sql` vs `0023_tidy_zaran.sql` + a follow-on `0024_rare_maestro.sql`); `SELECT id, hash, created_at FROM drizzle.__drizzle_migrations` confirmed ALL THREE were already applied there, tracked by content HASH, not filename/idx. That made `git mv 0023_tidy_zaran.sql 0024_tidy_zaran.sql` (+ its snapshot, + `0024_rare_maestro.sql`→`0025_rare_maestro.sql`) safe — zero SQL bytes touched, so the hash-based tracker doesn't care about the rename, and `pnpm db:migrate` after the merge applied with zero errors. The part that ISN'T just a rename: `meta/NNNN_snapshot.json` chains lineage via `id`/`prevId` UUIDs, independent of filename/idx — the renamed `0024_snapshot.json` (originally `0023_tidy_zaran`'s) still had `prevId` pointing at the OLD common-ancestor snapshot (`0022_sloppy_swordsman`'s id) after the rename, which is now wrong once `0023_flimsy_nick_fury` sits between them in the merged sequence. Had to hand-edit that one `prevId` field to point at `flimsy_nick_fury`'s snapshot `id` instead. Skipping this step wouldn't have broken `db:migrate` (which just replays `.sql` files in `_journal.json` array order) but would have left a wrong lineage for the next `pnpm db:generate` to diff against. General fix recipe for this collision: rename the `.sql` + snapshot files to the next free idx, bump `_journal.json`'s `idx`/`tag` to match, and repoint the renamed snapshot's `prevId` at whatever migration now immediately precedes it — three separate edits, and only the third one is easy to forget because `db:migrate` won't complain if you skip it.
